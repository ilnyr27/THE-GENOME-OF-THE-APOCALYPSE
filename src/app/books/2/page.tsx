...
import { book2Chapters } from "@/lib/data";
import { ReadingTracker } from "@/components/ReadingTracker";
import { LiveReaders } from "@/components/LiveReaders";
import { useReadingPosition } from "@/hooks/useReadingPosition";

export default function Book2ReaderPage() {
  const { currentChapter, goToChapter } = useReadingPosition(2, book2Chapters.length);
  const [sidebarOpen, setSidebarOpen] = useState(false);

  if (currentChapter === null) return null;

  const chapter = book2Chapters[currentChapter];
      ...
      <div className="fixed top-16 left-0 right-0 z-30 glass border-b border-bunker-800">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-ash-600">Книга 2</p>
              <p className="text-sm text-ash-300">Эволюция Пустоши</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <LiveReaders bookId={2} chapterId={chapter.id} />
            <p className="text-xs text-ash-600">
              Глава {currentChapter + 1} из {book2Chapters.length}
            </p>
          </div>
        </div>
        {/* Progress bar */}
        <div className="h-0.5 bg-bunker-900">
          <motion.div
            ...
            animate={{ width: `${((currentChapter + 1) / book2Chapters.length) * 100}%` }}
      ...
      <div className="flex">
        ...
        <div className="flex-1 pt-28 pb-24 px-4">
          <div className="max-w-2xl mx-auto">
            ...
            <ReadingTracker bookId={2} chapterId={chapter.id} />
            ...
            <div className="flex items-center justify-between mt-16 pt-8 border-t border-bunker-800">
              ...
              {currentChapter < book2Chapters.length - 1 ? (
                <button
